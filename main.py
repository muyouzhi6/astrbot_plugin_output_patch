...
class OutputPlugin(Star):
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        if result.is_llm_result() and self._is_empty_text_only_chain(result.chain):
            logger.debug(
                "[outputpro:on_decorating_result] skip empty LLM text result"
            )
            return
    ...
    @staticmethod
    def _is_empty_text_only_chain(chain) -> bool:
        if not chain:
            return True
        for comp in chain:
